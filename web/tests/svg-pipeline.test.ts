...
test("runCanonicalSvgPipeline removes figma ids, trivial clip paths, and flattens a safe root group", () => {
  const input = `
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
      <g
        id="Icons / Queue / Params"
        stroke="#121212"
        stroke-width="1.5"
        data-node-id="156:1986"
        clip-path="url(#clip0)"
      >
  ...
  assert.doesNotMatch(output, /clip-path=/)
  assert.doesNotMatch(output, /<defs[\s>]/)
...
test("runCanonicalSvgPipeline keeps non-trivial clip paths", () => {
  const input = `
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
      <g clip-path="url(#clip0)">
        <path d="M3 12h18" stroke="#121212" stroke-width="1.5" />
      </g>
      <defs>
        <clipPath id="clip0">
  ...
  assert.match(output, /clip-path="url\(#a\)"/)
  assert.match(output, /<defs>/)
  assert.match(output, /<clipPath id="a">/)
})
